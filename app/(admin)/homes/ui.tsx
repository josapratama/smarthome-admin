...
function CreateHomeDialog() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: HomeCreateRequest) => {
      return apiFetchBrowser("/api/homes", {
        method: "POST",
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: qk.homes() });
      setOpen(false);
      setFormData({
        name: "",
        ownerUserId: 1,
...
export function HomesClient() {
  ...
  const q = useQuery({
    queryKey: qk.homes(),
    queryFn: async () => {
      const payload = await apiFetchBrowser<HomesListResponse>("/api/homes");
      return payload.data ?? [];
    },
  });

  const filtered = (q.data ?? []).filter((home) => {
